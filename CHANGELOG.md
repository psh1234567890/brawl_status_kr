...
- README 공개 릴리스 버전을 0.1.3으로 맞추고, 로드맵의 구현 완료 항목과 향후 계획을 구분

## 0.1.3 - 2026-09-22

- 최근 전체 보유 스킨 브라우저 캐시가 있으면 조회 버튼 클릭 즉시 캐시를 먼저 표시하고, 뒤의 provider refresh가 204 또는 일시 실패하더라도 기존 캐시를 유지하도록 개선

## 0.1.2 - 2026-09-22

- 브라우저의 전체 보유 스킨 fallback 캐시를 24시간 만료와 최신 20개 태그 상한으로 제한해 장기 사용 시 localStorage 누적을 방지

## 0.1.1 - 2026-09-22

- 보유 스킨 버튼 클릭 즉시 이미 검색된 공식 플레이어 데이터로 착용 스킨 baseline을 표시하고, 서버에는 전체 보유 목록 확장만 별도로 요청하도록 개선. 보조 provider 성공 결과는 warm instance에서 6시간 fresh/24시간 stale 캐시로 재사용
- 보유 스킨 버튼은 공식 플레이어 API의 현재 착용 스킨을 안정적인 기본 결과로 반환하고, 명시적으로 활성화된 Brawlace/Jina 보조 provider가 성공할 때만 전체 보유 목록으로 확장하도록 변경
- 플레이어 검색과 보유 스킨 보조 조회를 분리해 기본 검색은 스킨 외부 조회를 기다리지 않으며, 브롤러 탭에서 사용자가 원할 때만 보유 스킨을 별도로 조회하도록 변경
