...
- 브롤러별 현재 장착 스킨, 보유 스킨 목록, 가젯, 스타파워, 하이퍼차지, 기어 표시
- 검색한 플레이어의 전투 기록을 PostgreSQL에 중복 없이 적재
- DB에 저장된 전체 전투 표본 중 맵별 최소 5판 이상 데이터를 기반으로 추천 점수 제공
...
메타 통계는 전체 브롤스타즈 이용자 통계가 아니라 이 서비스 DB에 저장된 전체 전투 표본입니다. 팀전은 저장된 전투 JSON의 양 팀 참가자를 펼쳐 계산하고, 쇼다운은 API가 상대별 순위를 제공하지 않아 검색된 플레이어 관점의 기록만 사용합니다.
