...
플레이어 검색에서 쌓인 전투 표본으로 전적, 보유 브롤러, 맵 추천, 팀 조합, 카운터를 제공하는 다국어 Brawl Stars 분석 서비스입니다.

- 라이브 서비스: <https://www.brawl-o1.site/>
- 프로젝트 상태: `v0.1.0` 정식 공개 릴리스
...
- 브라우저는 이미 검색된 공식 프로필에서 착용 스킨 baseline을 즉시 만들고, 서버에는 전체 보유 스킨 확장만 요청합니다. 활성화된 보조 provider의 성공 결과는 warm server instance에서 6시간 fresh/24시간 stale 캐시됩니다.
...
- 일반 2팀 전투는 같은 전투 지문을 한 번만 고른 뒤 양 팀 참가자를 펼쳐 계산합니다.
- 듀오·트리오 쇼다운은 여러 상대 팀의 결과를 임의로 만들지 않고 검색된 플레이어 관점의 순위를 사용합니다.
- 친선전은 통계에서 제외합니다.
...
| `BRAWLACE_SKIN_LOOKUP_ENABLED` | 선택 | `true`일 때만 Brawlace 보유 스킨 보조 조회 사용. 기본값은 비활성 |
| `BRAWLACE_JINA_READER_ENABLED` | 선택 | `true`일 때만 Brawlace 직접 조회 실패 시 Jina Reader fallback 사용. 기본값은 비활성 |
| `JINA_READER_API_KEY` | 선택 | opt-in Jina Reader 요청에만 보내는 서버 전용 API 키 |
...
일본어와 브라질 포르투갈어·스페인어·튀르키예어·독일어·프랑스어·이탈리아어·러시아어의 브롤러·맵·모드·설명·능력·스킨 표시명 카탈로그는 다음 명령으로 BrawlAPI 원본에서 다시 생성할 수 있습니다.

```powershell
npm.cmd run translations:additional:update
```

현재 10개 언어 지원 범위는 전적 검색, 맵 메타, 팀 조합, 카운터, 랭킹, 수집 현황뿐 아니라 맵·브롤러·게임모드 도감과 상세 화면, 이벤트 로테이션, 스킨 카탈로그, 클럽 검색, 소개·데이터 산정 방식·개인정보처리방침·이용 안내·문의 페이지까지 포함합니다. 한국어 기본 URL은 유지하며 각 언어는 `/en`, `/ja`, `/pt-br` 같은 접두 경로를 사용합니다. 루트(`/`) 첫 방문에서는 저장된 언어 선택을 우선하고, 저장값이 없으면 `Accept-Language`를 사용합니다. 지원하지 않는 브라우저 언어는 영어로 안내합니다.
...
현재 우선순위는 정식 릴리스 이후 운영 안정성·Search Console 색인 추이·실제 피드백·사용량 근거를 관찰하고, 제3자 조건과 의존성을 정기적으로 재확인하는 것입니다. 계획과 현재 기능을 섞지 않으며 자세한 내용은 [ROADMAP.md](docs/ROADMAP.md)와 [CHANGELOG.md](CHANGELOG.md)에 있습니다.
