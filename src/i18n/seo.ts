...
export type LocalizedSiteSeo = Readonly<{
  title: string;
  description: string;
  openGraphLocale: string;
}>;

const localizedSiteSeo: Record<LocalizedLocale, LocalizedSiteSeo> = {
  en: {
...
export function getLocalizedSiteSeo(locale: LocalizedLocale) {
  return localizedSiteSeo[locale];
}

export function getLocalizedSocialMetadata(locale: LocalizedLocale): Pick<Metadata, "openGraph" | "twitter"> {
  const { title, description, openGraphLocale } = getLocalizedSiteSeo(locale);
  const socialTitle = `${title} | Brawl Status KR`;
